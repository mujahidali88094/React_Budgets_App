...
export default function ViewBudgetModal({ show, handleClose, budgetId }) {
  const { budgets, getBudgetExpenses, removeExpense, removeBudget } =
    useBudgetsContextValue();
  if (budgetId === "") return null;
  let budget = {};

  if (budgetId === UNCATEGORIZED_ID) budget.name = "Uncategorized";
  else {
  ...
  return (
    <Modal show={show} onHide={handleClose}>
      <Modal.Header closeButton className="d-flex flex-row align-items-center">
      ...
      <Modal.Footer className="justify-content-between">
        {budget.max && <Card className="px-3 py-1">Limit: ${budget.max}</Card>}
